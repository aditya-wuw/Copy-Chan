        ...
        {Filtered.length > 0 && symbol ? (
          Object.entries(symbol).map(([sections, items]) => (
            <div key={sections}>
              ...
              <div className={`${sections === "Others" ? "grid grid-cols-5 text-xs": "grid grid-cols-7" } grid-rows-1 gap-2`}>
                {items.map((i) => (
                  <button
                  key={i.label}
                  ...
                  >
                    {i.emoji}
                    <span className={`opacity-0 text-xs pointer-events-none group-hover:opacity-100 absolute ${sections === "Others" ? "bottom-13":"bottom-11"} left-0 right-10 bg-blue-500  text-white flex justify-center items-center z-10 rounded-md p-2 px-2 w-full line-clamp-1`}>
                      {i.label}
                    </span>
                  </button>
                ))}
          ...
          <div className="mt-1 flex justify-center">no data</div>
        )}
      </div>
    </main>
  );
